...
var JsCadViewerView = require('./jscad-viewer-view');
...
module.exports = {
  config: configScheme,
  jsCadViewerView: null,
  modalPanel: null,
  subscriptions: null,
  pane: null, // pane which contains the view; useful to check event listeners
  // notification: null,
  activate: function ( /* state */ ) {
    // console.log('jscad-viewer.activate');

    this.subscriptions = new CompositeDisposable;

    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'jscad-viewer:toggle': this.toggle.bind(this)
    }));

    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'jscad-viewer:preview-file': this.previewFile.bind(this)
    }));

    this.subscriptions.add(atom.workspace.observeActivePaneItem(function ( /* paneItem */ ) {
      // console.log('jscad-viewer.subscription.observeActivePaneItem', paneItem);
      ...
      allPaneItems.forEach(function (paneItem, idx) {
        if (paneItem instanceof JsCadViewerView && paneItem.pathname === activeFile) {
          previewItemIdx = idx;
        }
      });
    ...
    return atom.workspace.addOpener(function (uriToOpen) {
      // console.log('jscad-viewer.addOpener', uriToOpen);
      ...
      this.jsCadViewerView = new JsCadViewerView(pathname);
      return this.jsCadViewerView;
      /*
  if (host === 'editor') {
  ...
  */
  ...
  deactivate: function () {
    ...
    return this.jsCadViewerView && this.jsCadViewerView.destroy();
  },
  serialize: function () {
    if (!this.jsCadViewerView)
      return null;
    return {
      jsCadViewerViewState: this.jsCadViewerView.serialize()
    };
  },
  /**
   ...
   * @return {TextEditor}    A promise that resolves to the `TextEditor` of the jscad-viewer.
   */
  preview: function (editor, uri) {
    return atom.workspace.open(uri, {
      ...
      view.events.on('error', function (err) {
        ...
        console.error('jscad-viewer.preview ERROR:', err);
   ...
   */
  toggle: function () {
    console.log('JsCadViewer.toggle');
    ...
    // console.error('jscad-viewer.toggle', uri);
  ...
  previewFile: function previewFile(e) {
    ...
    // console.warn('jscad-viewer.previewFile', selection, editor && editor.getTitle() || 'no editor');
    this.preview(editor, 'jscad-viewer://' + selection).then(function (view) {
