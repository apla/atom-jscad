...
module.exports = AtomScadPreview = {
  ...
  activate: function(state) {
    ...
    ['jscad', 'scad'].forEach (function (formatName) {
      this.subscriptions.add(atom.commands.add('.tree-view .file .name[data-name$=\\.'+formatName+']', {
        'atom-scad-preview:toggle': this.previewFile.bind (this)
      }));
    }, this);
    ...
    this.subscriptions.add(atom.workspace.observeActivePaneItem(function (paneItem) {
      ...
      console.log ('active pane change', paneItem);
  ...
  toggle: function() {
    console.log('AtomScadPreview was toggled!');
      ...
      atom.workspace.open( uri, { split: 'right', searchAllPanes: true } ).then(function( view ) {
  ...
  config: {
    drawLines: {
      title: 'Draw lines',
      description: 'Draw black triangle lines ("wireframe")',
      type: 'boolean',
      'default': false
    },
    drawFaces: {
