...
module.exports = AtomScadPreviewView = (function() {
  ...
  function AtomScadPreviewView( pathname ) {
    ...
    var currentFile = new AtomFile (pathname, false);

    this.includesCache = {};

    this.subscriptions.add (currentFile.onDidChange (this.onFileChange.bind (this, pathname)));
    ...
    this.element.classList.add('atom-jscad-preview');
  ...
  AtomScadPreviewView.prototype.onFileChange = function (filename, fileHandler) {

    // checks main file for modifications
    if (filename === this.pathname) {
      var fileContents = fs.readFileSync (filename).toString();
      if (this.mainCache !== fileContents) {
        this.mainCache = fileContents;
        this.render ();
      }
      return;
    }

    if (filename in this.includesCache) {
      var fileContents = fs.readFileSync (filename).toString();
      if (this.includesCache[filename] !== fileContents) {
        this.includesCache[filename] = fileContents;
        this.render ();
      }
    } else {
      // file is removed from includes, but we still have watcher
      // TODO: remove the watcher
    }

  }

  AtomScadPreviewView.prototype.removeNotifications = function () {
    atom.notifications.getNotifications().forEach (function (notification) {
        if (notification.options.origin === originToken) {
  ...
  AtomScadPreviewView.prototype.render = function () {
    ...
    var jscadscript = this.mainCache || fs.readFileSync (this.pathname).toString();

    this.removeNotifications ();
    var pathChunks = OpenJsCad.splitFilename (this.pathname);
    var sandbox;
    try {
      sandbox = OpenJsCad.getParamDefinitions (jscadscript, {includesDir: pathChunks.dirname}, true);
    } catch (e) {

    }

    if (sandbox && sandbox.includes) {
      sandbox.includes.forEach (inc => {

        if (!(inc in this.includesCache)) {
          console.log ('adding subscription', inc);
          var incFile = new AtomFile (inc.path, false);
          this.subscriptions.add (incFile.onDidChange (this.onFileChange.bind (this, inc.path)));
          this.includesCache[inc.path] = null;
        }
      })
    }
  ...
  AtomScadPreviewView.prototype.loadDependencies = function () {
    ...
    function dependencyLoaded (idx) {
      ...
      if (depsToLoad === 0) {
        ...
        var modelColor = atom.config.get(packageName + '.color');
        var bgColor    = atom.config.get(packageName + '.backgroundColor');

        var gProcessor = new OpenJsCad.Processor (this.viewer, {
          drawLines: atom.config.get(packageName + '.drawLines'),
          drawFaces: atom.config.get(packageName + '.drawFaces'),
          drawGrid:  atom.config.get(packageName + '.drawGrid'),
  ...
  AtomScadPreviewView.prototype.getURI = function() {
    return 'jscad-preview://' + this.pathname;
  };
