...
OpenJsCad.Viewer.ThreeEngine.prototype = {
  ...
  render: function () {
    ...
    if (!this.pauseRender_) {
      ...
      try {
        ...
        console.error('render error', e, this.scene_, this.camera_);
      }
    }
  },
  animate: function () {
    ...
    'x y z'.split(' ').forEach(function (axis) {
      ...
      this.scene_.add(new THREE.Line(
        ...
        new THREE.LineBasicMaterial({
          color: rgbTo24Bit(axes[axis].pos),
          opacity: axes[axis].pos.a,
          transparent: true,
          lineWidth: 1
        })
      ));
      ...
      this.scene_.add(new THREE.Line(
        lineNeg,
        new THREE.LineBasicMaterial({
          color: rgbTo24Bit(axes[axis].neg),
          opacity: axes[axis].neg.a,
          transparent: true,
          lineWidth: 1
        })
      ));
  ...
  handleResize: function () {
    ...
    this.resizeCanvas(canvas, function cb() {
      self.camera_.aspect = canvas.width / canvas.height;
      // console.log('handleResize', canvas.width, canvas.height, self.camera_.aspect);
      self.camera_.updateProjectionMatrix();
      // set canvas attributes (false => don't set css)
      self.renderer_.setSize(canvas.width, canvas.height, false);
      self.render();
    });
